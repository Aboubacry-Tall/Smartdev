...
class CrmLead(models.Model):
    ...
    def get_view(self, view_id=None, view_type="form", **options):
        ...
        if (self._dg_read_all_active() or self._siha_agent_read_only() or self._finance_read_only() or self._dh_read_only()) and view_type in {"list", "kanban", "form"}:
    ...
    def create(self, vals_list):
        if self._dg_read_all_active() or self._siha_agent_read_only() or self._finance_read_only() or self._dh_read_only():
    ...
    def unlink(self):
        ...
        if self._dh_read_only():
            raise AccessError(_("Le groupe DH n'est pas autorisé à supprimer des leads/opportunités."))
